...
def dataset_poster_lookup():
    ...
    if df is None or "title" not in df.columns:
        return {}
    working = df.copy()
    if "poster_url" in working.columns:
        source_column = "poster_url"
    else:
        source_column = "poster_path"
    ...
    lookup = (
        ...
        .set_index("title_key")[source_column]
        .to_dict()
    )
...
def _poster_url_from_movie(movie) -> str | None:
    if movie is None:
        return None
    poster_url = movie.get("poster_url")
    if isinstance(poster_url, str) and poster_url.strip():
        return poster_url
    return _poster_url_from_path(movie.get("poster_path"))
...
def fetch_poster(title: str = "", movie=None, df=None) -> str:
    if movie is not None:
        poster_url = _poster_url_from_movie(movie)
        if poster_url:
            return poster_url
    ...
    if df is not None and title:
        ...
        if matched_row is not None:
            poster_url = _poster_url_from_movie(matched_row)
            if poster_url:
                return poster_url

    if title:
        poster_path = dataset_poster_lookup().get(str(title).lower().strip())
        poster_url = poster_path if isinstance(poster_path, str) and poster_path.startswith("http") else _poster_url_from_path(poster_path)
        if poster_url:
            return poster_url
